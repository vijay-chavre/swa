...
export default function* fetchUser() {
  while (true) {
    const fetchAction = yield take(UserActions.FETCH_LEADERS);
      ...
      var url= `/leaderboards/${ENV.leaderboardName}/documents?`;   
        if (fetchAction.payload.isThisMonth === 'true') { // for outstanding
            if (fetchAction.payload.isMyCountry === 'true') {
                ...
                url += `limit=3&sort%5BrewardPoints.outstanding%5D=desc`;
            }
            else if (fetchAction.payload.isMyGrade === 'true') {
                url += `filter%5Bgrade%5D%5B%3D%5D=${grade}&limit=3&sort%5BrewardPoints.outstanding%5D=desc`;
            }
            else {
                url += `limit=3&sort%5BrewardPoints.outstanding%5D=desc`;
            }
            ...
            if (fetchAction.payload.isMyCountry === 'true'){
                ...
                url += `limit=3&sort%5BrewardPoints.lifetime%5D=desc`;
            }
            else if (fetchAction.payload.isMyGrade === 'true') {
                url += `filter%5Bgrade%5D%5B%3D%5D=${grade}&limit=3&sort%5BrewardPoints.lifetime%5D=desc`;
            }
            else {
                url += `limit=3&sort%5BrewardPoints.lifetime%5D=desc`;
            }
    ...
    try {
      ...
      console.log("userRespionse"+userResponse);
      yield put({ type: 'SET_LOADING', isLoading: false });
      yield put(UserActions.leadersFetched(userResponse));
      console.log("userResponse =" + userResponse);
    } catch (error) {
      Raven.captureException(error);
      yield put({ type: 'SET_LOADING', isLoading: false });
      yield put(UserActions.fetchLeadersFailed(error));
    }
  }
}

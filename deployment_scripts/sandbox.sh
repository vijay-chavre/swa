...
RELEASE=SANDBOX_$(git rev-parse --short HEAD)
echo $RELEASE
NODE_ENV=sandbox RELEASE=$RELEASE  webpack --config ./config/webpack.config.prod.js --progress
...
sed -i -e s/ENV_ENVIRONMENT/sandbox/g dist/index.html 

gzip -r -9 dist/
for f in `find dist/ -iname '*.gz'`; do
  mv $f ${f%.gz} 
done
aws s3 sync dist/ s3://studentapp-sandbox.hellothinkster.com --content-encoding gzip --cache-control public,max-age=30672000